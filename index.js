...
(function(global) {
  ...
  function main() {
    ...
    function kubus(){
      ...
      var ambientColor = glMatrix.vec3.fromValues(0.17, 0.01, 0.20);
      gl.uniform3fv(lightColorLoc, lightColor);
      gl.uniform3fv(lightPositionLoc, moving);
    ...
    function huruf(){
      ...
      var hurufVBO = gl.createBuffer();
      gl.bindBuffer(gl.ARRAY_BUFFER, hurufVBO);
      gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(hurufVertices),gl.STATIC_DRAW);
      ...
      if (moving[0]+(-0.05) < (-0.5*1.5) || moving[0]+(-0.05) > (0.5*1.5)) {
        xAdd *= -1;
      }
      ...
      if (moving[1]+(-0.5)<(-0.5*1.5) || moving[1]+0.5>(0.5*1.5)) {
        yAdd *= -1;
      }
